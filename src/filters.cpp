...
//     // Precompute response storage
//     int totalResponses = filterBank.size() * 3;  // 3 channels
//     std::vector<cv::Mat> filterResponses(totalResponses);

//     // Parallelize over filter-channel combinations
//     #pragma omp parallel for collapse(2) schedule(dynamic)
//     for (int filterIdx = 0; filterIdx < filterBank.size(); ++filterIdx) {
//         for (int channelIdx = 0; channelIdx < 3; ++channelIdx) {
//             const auto& filter = filterBank[filterIdx];
//             const cv::Mat& channel = (channelIdx == 0) ? L_channel : (channelIdx == 1) ? a_channel : b_channel;

//             cv::Mat response;
//             cv::filter2D(channel, response, CV_32F, filter);

//             // Save response
//             saveFilterResponseImage(response, outputPath, filterIdx + 1, (channelIdx == 0) ? "L" : (channelIdx == 1) ? "a" : "b");

//             // Store response
//             filterResponses[filterIdx * 3 + channelIdx] = response;
//         }
//     }
